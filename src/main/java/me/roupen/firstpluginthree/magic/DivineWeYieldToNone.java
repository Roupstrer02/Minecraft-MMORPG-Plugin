...
public class DivineWeYieldToNone extends BukkitRunnable {
    ...
    public void spellStartup() {
        ...
        Targets = loc.getNearbyPlayers(SpellAOE(7));
        spells.ParticleCircle(loc, SpellAOE(7), Particle.WAX_ON, false);
        for (Player player: Targets) {
            if (!player.getName().equals(origin.getName())) {
                ...
                TargetStats.changeMultiplicativeStats("Mana Regen", StatIncreaseFactor());
            }
        ...
        for (Player player: Targets) {
            if (!player.getName().equals(origin.getName())) {
                ...
                if (progress == spellCooldown - 1) {
                    ...
                    TargetStats.changeMultiplicativeStats("Mana Regen", 1 / StatIncreaseFactor());
                }
            }
        }
    }
